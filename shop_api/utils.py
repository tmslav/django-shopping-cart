...
def open_in_browser(res):
    ...
    with open("temp.html","w") as f:
        f.write(res.text.encode("utf8","ignore"))
    webbrowser.open_new_tab("file://"+os.path.abspath("temp.html"))
...
class BaseCrawler:
    ...
    def check_cart_number(self,url,cookie):
        res = self.session.get(url,cookies=cookie,headers=login_headers)
        product_number = re.findall(self.product_number_regex,res.text)
        if product_number:
            return int(product_number[0])
        else:
            return -1
...
class JumboCrawler(BaseCrawler):
    ...
    def add_product(self,product, user_cookie,quantity=1):
        ...
        res = self.session.post(post_url,data=data,headers=headers,cookies=cookie)
    ...
    def login(self, user_name, password):
        #navigate to page
        res1 = self.session.get(self.base_url)
        ...
        first_res = self.session.get(first_req,headers = first_req_headers)
        ...
        sec_res = self.session.get(sec_req,headers=first_req_headers)
        ...
        redirect = self.session.post(login_url,data=form_login,headers=self.headers,cookies = res1.cookies,allow_redirects=False)
        res = self.session.get(urljoin(login_url,redirect.headers['Location']),headers = self.headers,cookies=redirect.cookies)
        ret_cookies = {}
...
class AhCrawler(BaseCrawler):
    ...
    def add_product(self,product_id, user_cookie,quantity=1):
        add_product_headers = {
        ...
        res = self.session.post(add_product_url, data=post_data,
    ...
    def login(self, user_name, password):
        ...
        res = self.session.post(self.base_url, data=form_data, cookies=res.cookies, headers=self.headers)
        ...
        res = self.session.get(next_url, cookies=res.cookies, headers=self.headers)
        ...
        res = self.session.get(next_url, cookies=login_cookie, headers=self.headers)
...
class HoogvlietCrawler(BaseCrawler):
    ...
    def login(self,user_name,password):
        ...
        self.login_cookie = deepcopy((dict(redirect.request.headers),token))

    def add_product(self,product,cookie,quantity=1):
        url = "https://www.hoogvliet.com/INTERSHOP/web/WFS/org-webshop-Site/nl_NL/-/EUR/ViewCart-AddProduct"
        cookie,token = cookie

        headers = deepcopy(cart_headers)
        headers['SynchronizerToken'] = token
        product_number = self.check_cart_number(self.login_url,cookie)
        ...
        res = self.session.post(url,data=post_data,cookies=cookie,headers=headers)
        new_product_number = self.check_cart_number(self.login_url,cookie)

        if product_number<new_product_number:
            return True
        else:
            return False

class CooponlineCrawler(BaseCrawler):
    base_url = "https://www.cooponline.nl"
    ...
    def add_product(self,product,user_cookie,quantity=1):
        ...
        res = self.session.post(self.add_product_url.format(product,quantity,basket),data=post_body,headers=headers,cookies=cookie.get_dict())
